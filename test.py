...
# 根据csv列表，读取信息并写入数组中。
# [4, 18, 20, 22, 25, 33]
# {'learnrate': 0.094, 'posibility': 
# {'16': -0.259, '2': -0.734, '1': -0.682, '33': 0.849, '15': 0.308, '26': -0.504, '23': 0.416, 
# '32': -0.504, '14': -0.569, '25': 0.762, '21': -0.432, '31': -0.362, '12': 0.357, '29': -0.395,
# '13': -0.85, '4': 0.73, '7': -0.468, '24': 0.442, '3': 0.096, '11': 0.126, '17': -0.214, 
# '27': -0.194, '10': 0.614, '8': -0.428, '5': 0.192, '18': 0.713, '22': 0.878, '19': -0.391, 
# '20': 0.638, '9': 0.05, '28': 0.126, '6': -0.325, '30': 0.13}, 'threshole': 0.879}
...
def main():
	...
	study['threshole']=6.09
	study['learnrate']=0.05

	study['posibility'] = initNumPossibility()
	predict = model(i_data[0:len(i_data)-2],study)
	fact = i_data[len(i_data)-1][0:6]
	sameCount = comparePreAndFac(predict,fact)
	print(sameCount)

	outpre = model(i_data,study)
